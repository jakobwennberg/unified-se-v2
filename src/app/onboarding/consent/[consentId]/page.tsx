...
const PROVIDER_LOGOS: Record<string, string | null> = {
  fortnox: '/providers/FNOX.ST.svg',
  visma: '/providers/visma-logo.svg',
  briox: '/providers/Briox_logo.png',
  bokio: '/providers/bokio-logo.png',
  bjornlunden: '/providers/bjorn-lunden-logo.png',
  'manual-sie': null,
};
...
export default function OnboardingPage() {
  ...
  const effectiveProvider = consent?.provider ?? selectedProvider;
  ...
  const handleAccept = async () => {
    if (!consent || !effectiveProvider) return;
    setAccepting(true);
    setError(null);
    ...
    try {
      ...
      const state = otcCode
        ? `${effectiveProvider}:${consentId}:${otcCode}`
        : `${effectiveProvider}:${consentId}`;
      const res = await fetch(
        `/api/v1/auth/${effectiveProvider}/url?state=${encodeURIComponent(state)}`,
      );
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-xl border border-border/60 bg-card p-8 shadow-[0_4px_24px_rgba(0,0,0,0.25)] space-y-6">
        ...
        {error && (
          <div className="rounded-lg border border-destructive/20 bg-destructive/10 p-4 text-sm text-[#f87171]">
            {error}
          </div>
        )}

        {consent && !done && !effectiveProvider && (
          <>
            <div className="space-y-2 text-center">
              <h1 className="font-serif text-xl tracking-tight">Choose Your Accounting System</h1>
              <p className="text-sm text-muted-foreground">
                Select which bookkeeping system you&apos;d like to connect.
              </p>
            </div>
              ...
              {Object.entries(PROVIDER_LABELS).map(([key, label]) => {
                ...
                return (
                  <button
                    ...
                    className="w-full rounded-lg border border-border/60 bg-accent/30 px-4 py-3 text-sm font-medium text-foreground transition-all duration-200 hover:border-primary/50 hover:bg-accent/50 text-left flex items-center gap-3"
                  >
                    {logo ? (
        ...
        {consent && !done && effectiveProvider && (
          <>
            <div className="space-y-2 text-center">
              <h1 className="font-serif text-xl tracking-tight">Authorize Access</h1>
              <p className="text-sm text-muted-foreground">
                You have been invited to connect your{' '}
                <strong>{PROVIDER_LABELS[effectiveProvider] ?? effectiveProvider}</strong>{' '}
                accounting system.
              </p>
            </div>

            <div className="rounded-lg border border-border/60 bg-accent/30 p-4 space-y-2 text-sm">
              ...
              {otcCode && (
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-xs text-[#3fb950]">Valid</span>
                </div>
              )}
            </div>

            {effectiveProvider === 'briox' ? (
              <>
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    className="w-full rounded-md border border-input bg-[#0d1321] px-3 py-2 text-sm text-foreground transition-colors duration-200 placeholder:text-muted-foreground/40 focus:outline-none focus:ring-2 focus:ring-ring/40 focus:border-primary/50"
                  />
                </div>
              ...
              <>
                <div className="space-y-3">
                  ...
                  <ol className="text-xs text-muted-foreground list-decimal list-inside space-y-1">
                    ...
                    <li>Find your <strong>Company ID</strong> in the Bokio URL &mdash; it&apos;s the ID after <code className="bg-accent px-1 rounded text-foreground">app.bokio.se/</code></li>
            ...
            {!consent.provider && (
              ...
              >
                &larr; Choose a different system
              </button>
            )}
          </>
        )}
        ...
        {done && (
          <div className="space-y-4 text-center">
            ...
            <h2 className="font-serif text-xl tracking-tight">
              {effectiveProvider === 'manual-sie' ? 'File Uploaded!' : 'Connected!'}
            </h2>
            {sieUploadResult ? (
              <>
                <p className="text-sm text-muted-foreground">
                ...
                <div className="rounded-lg border border-border/60 bg-accent/30 p-3 text-sm space-y-1">
                  <div className="flex justify-between">
                ...
                <button
                  ...
                  className="w-full rounded-md border border-input bg-transparent px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent"
                >
                  Upload Another File
                </button>
              </>
